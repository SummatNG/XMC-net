...
from utils import MF_DATA, CausE_DATA, evaluate_model,XMC_Net_DATA
import numpy as np
import argparse
import random
import torch
...
# XMC-net
def train_XMC_Net(propensity_score):
    train_data = XMC_Net_DATA(opt.XMC_train_data , opt.XMC_val1_data)
    val_data = MF_DATA(opt.XMC_val2_data)
    train_dataloader_s_c = DataLoader(train_data.s_c,
                                      opt.batch_size,
                                      shuffle=True)
    train_dataloader_s_t = DataLoader(train_data.s_t,
    ...
    model = getattr(models,
                    opt.model)(inverse_propensity, train_data.users_num, train_data.movies_num ,train_data.items_num,
                               opt.embedding_size, opt.reg_c, opt.reg_c,
                               opt.reg_tc, train_data.s_c[:, :2].tolist(),
                               train_data.s_t[:, :2].tolist())
    ...
    # columns = np.loadtxt('/data/train/taotalDt_columns.txt')
    for epoch in range(opt.max_epoch):
        t1 = time()
        for i, data in tqdm(enumerate(train_dataloader_s_c)):
            # train model
            ...
            tag_user_list = data[:, 0].to(opt.device)
            tag_list = data[:,1].to(opt.device)
            tag_label_list = data[:, 2].to(opt.device)

            rating_user_list = data[:, 0].to(opt.device)
            item_list = data[:, 3].to(opt.device) #movie
            tag_index_list = data[:, 6:14].to(opt.device)
            rating_label_list = data[:, 4].to(opt.device)

            ips_tag = data[:, 14].to(opt.device)
            ips_movie = data[:, 5].to(opt.device)

            loss = model.calculate_loss(1,
                                        tag_user_list.long(),
                                        tag_list.long(),
                                        tag_label_list.float(),
                                        ips_tag,
                                        ips_movie,
                                        rating_user_list.long(),
                                        item_list.long(),
                                        tag_index_list.long(),
                                        rating_label_list.float()
                                        )
...
if __name__ == '__main__':
    ...
    # 添加参数
    parser.add_argument('--model', default='XMC_Net')
    ...
    if opt.model == 'MF_IPS' or opt.model == 'MF_Naive':
        ...
        generate_submit(best_model)
    elif opt.model == 'XMC_Net':
        propensity_score = cal_propensity_score()
        propensity_score = np.array(propensity_score).astype(float)
        best_model = train_XMC_Net(propensity_score)
